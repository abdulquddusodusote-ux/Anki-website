...
//  Authentic Anki SM-2 Algorithm, Real-Time Time-Based Learning Queue,
//  Collapsible Subdecks, Study Time Tracker & Gemini 3.7 AI
// ================================================================
...
const DEFAULT_SETTINGS = {
    ...
    reviewedToday: 0,
    goalMetToday: false,
    totalStudyTimeToday: 0,         // stored in seconds
...
const App = {
    ...
    sessionNewQueue: [],
    sessionReviewQueue: [],
    learningPool: new Set(),        // Set of card IDs in learning state for dynamic re-evaluation
    currentCardId: null,
...
function formatStudyTimeDisplay(totalSeconds) {
...
// ================================================================
//  ANKI SM-2 SCHEDULER & ANKIDROID ACCURATE INTERVAL CALCULATION
// ================================================================
...
function previewIntervals(card) {
    ...
    // ---- 1. BRAND NEW CARD (Never failed, never graduated) ----
    if (cardState === 'new' && reps === 0 && lapses === 0) {
        return {
            again: '<1m',
            hard:  '<6m',
            good:  '<10m',
    ...
    // ---- 2. LEARNING CARD (In learning steps, after Good or previous Again) ----
    if (cardState === 'learning' || (cardState === 'new' && (reps > 0 || lapses > 0))) {
        // If card is on step 0 (e.g. repeated Again)
        if (reps === 0) {
            const scaledEasy = lapses > 0 ? '2d' : '3d';
            return {
                again: '<1m',
                hard:  '<6m',
                good:  '<10m',
...
function scheduleCard(card, rating) {
    ...
    if (state === 'relearning') {
        if (rating === 0) { // Again
            ...
            interval = 10 / (24 * 60);
        } else if (rating === 2) { // Good -> Re-graduate at 50% previous interval
            state = 'review';
            interval = Math.max(1, Math.round((card.interval || 1) * 0.5));
            reps++;
        } else if (rating === 3) { // Easy -> Re-graduate at 75% previous interval
            state = 'review';
            interval = Math.max(2, Math.round((card.interval || 1) * 0.75));
            reps++;
        }
    ...
    if (state === 'new' || state === 'learning') {
        ...
        if (rating === 0) { // Again
            ...
            // Graduate immediately: 4 days if brand new, 2-3 days if card had previous lapses
            state = 'review';
            interval = (lapses > 0 || reps > 0) ? Math.max(2, Math.round((s.easyInterval || 4) * 0.5)) : (s.easyInterval || 4);
            reps = learningSteps.length;
        }
    ...
    if (state === 'review') {
        ...
        if (rating === 0) { // Again (Lapse back to relearning)
            newLapses++;
...
function hasSubdecks(deck) {
    if (!deck) return false;
    const prefix = deck.name + '::';
    return App.decks.some(d => d.name.startsWith(prefix) || d.parentId === deck.id);
}

function isDeckHiddenByParent(deck) {
    if (!deck) return false;

    // Check by parentId
    if (deck.parentId) {
        const parent = App.decks.find(d => d.id === deck.parentId);
        if (parent && parent.collapsed) return true;
        if (parent && isDeckHiddenByParent(parent)) return true;
    }

    // Check by name hierarchy (::)
    const parts = deck.name.split('::');
    if (parts.length > 1) {
        for (let i = 1; i < parts.length; i++) {
            const ancestorName = parts.slice(0, i).join('::');
            const ancestor = App.decks.find(d => d.name === ancestorName);
...
async function toggleDeckCollapse(deckId) {
    const deck = App.decks.find(d => d.id === deckId);
    if (!deck) return;
...
// ================================================================
//  AUTHENTIC ANKI REVIEW FLOW WITH REAL-TIME TIME-BASED INTERLEAVING
// ================================================================
...
function flushSessionStudyTime() {
    if (App.sessionElapsedSeconds > 0) {
        App.settings.totalStudyTimeToday = (App.settings.totalStudyTimeToday || 0) + App.sessionElapsedSeconds;
        App.sessionElapsedSeconds = 0;
...
// ----------------------------------------------------------------
//  getNextReviewCard() — AnkiDroid Real-Time Priority Engine
//
//  Priority Order:
//    1. Intraday learning cards whose due time has ARRIVED (now >= due)
//       -> HIGHEST PRIORITY! (Sorted by earliest due time)
//    2. Review cards due today
//    3. New cards
//    4. Pending learning cards in the near future (waiting screen / learn ahead)
// ----------------------------------------------------------------

function getNextReviewCard() {
    const now = Date.now();

    // 1. Check Intraday Learning Cards DUE RIGHT NOW (Top Priority)
    const dueLearningCards = [];
    for (const id of App.learningPool) {
        const card = App.cards.find(c => c.id === id);
        if (card && (card.state === 'learning' || card.state === 'relearning') && !card.suspended && !card.buried) {
            const dueTime = new Date(card.due).getTime();
            if (dueTime <= now) {
                dueLearningCards.push({ card, dueTime });
            }
        }
    }

    if (dueLearningCards.length > 0) {
        // Sort by earliest due time so oldest due learning card is reviewed first
        dueLearningCards.sort((a, b) => a.dueTime - b.dueTime);
        return dueLearningCards[0].card;
    }
    ...
    while (App.sessionReviewQueue.length > 0) {
        ...
        const card = App.cards.find(c => c.id === id);
        if (card && card.state === 'review' && !card.suspended && !card.buried && new Date(card.due).getTime() <= now) {
            return card;
        }
    ...
    // 4. If nothing is due right now, check if there are future learning cards
    const futureLearning = [];
    for (const id of App.learningPool) {
        const card = App.cards.find(c => c.id === id);
        if (card && (card.state === 'learning' || card.state === 'relearning') && !card.suspended && !card.buried) {
            futureLearning.push({ card, dueTime: new Date(card.due).getTime() });
        }
    }

    if (futureLearning.length > 0) {
        futureLearning.sort((a, b) => a.dueTime - b.dueTime);
        const soonest = futureLearning[0];
        return { waiting: true, card: soonest.card, dueTime: soonest.dueTime };
    }
...
function renderReviewCard() {
    ...
    // No cards left in session -> End Session
    if (!next) {
        endReviewSession();
        return;
    }

    // Waiting Screen State (Intraday learning cards waiting for timer)
    if (next.waiting) {
...
function endReviewSession() {
    ...
    // Save study time accumulated during review
    flushSessionStudyTime();
...
function updateStreakAndGoal() {
    ...
    if (last !== today) {
        ...
        App.settings.totalStudyTimeToday = 0; // reset daily study timer for new day
    } else {
...
function renderDecks() {
    ...
    sortedDecks.forEach(deck => {
        // If this deck's ancestor is collapsed, hide it
        if (isDeckHiddenByParent(deck)) {
            return;
        }

        const counts = getCardCounts(deck.id);
        const depth = (deck.name.match(/::/g) || []).length;
        const hasChildren = hasSubdecks(deck);
        ...
        // Collapse / Expand toggle chevron (Anki Desktop style)
        let toggleChevronHtml = '';
        if (hasChildren) {
            const iconClass = deck.collapsed ? 'fa-chevron-right' : 'fa-chevron-down';
            toggleChevronHtml = `<button class="deck-toggle-btn" data-toggle="${deck.id}" title="${deck.collapsed ? 'Expand subdecks' : 'Collapse subdecks'}"><i class="fas ${iconClass}"></i></button>`;
        } else if (depth > 0) {
            toggleChevronHtml = `<span class="deck-indent-spacer"></span>`;
        }

        cardEl.innerHTML = `
            <div class="deck-item-left">
                ${toggleChevronHtml}
                <div class="deck-title-click" data-study="${deck.id}">
            ...
            <div class="deck-item-right">
                <div class="deck-badges">
                    <span class="count-pill blue" title="New cards">${counts.blue}</span>
                    <span class="count-pill red" title="Learning cards">${counts.red}</span>
                    <span class="count-pill green" title="Due cards">${counts.green}</span>
                </div>
        ...
        // Click on deck title to study
        cardEl.querySelector('.deck-title-click')?.addEventListener('click', (e) => {
            e.stopPropagation();
            startReview(deck.id);
        });

        // Click on Study button
        cardEl.querySelector('.btn-study-deck')?.addEventListener('click', (e) => {
            e.stopPropagation();
            startReview(deck.id);
        });
...
function renderDashboard() {
    ...
    // Study Time Today (Fixed & Accurate)
    const studyTimeEl = document.getElementById('todayStudyTimeDisplay');
    if (studyTimeEl) {
        studyTimeEl.textContent = formatStudyTimeDisplay(s.totalStudyTimeToday || 0);
    }

    // Retention Rate from history
...
function renderBrowser() {
    ...
    const allowedDeckIds = filterDeckId ? getDescendantIds(filterDeckId) : null;

    let filtered = App.cards.filter(c => {
        if (allowedDeckIds && !allowedDeckIds.includes(c.deckId)) return false;
...
function setupEventListeners() {
    ...
    document.getElementById('generateAiCardsBtn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            btn.innerHTML = '<i class="fas fa-wand-magic-sparkles"></i> Generate High-Yield Flashcards';
            btn.disabled = false;
        }
    });
