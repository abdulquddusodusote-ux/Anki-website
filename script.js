...
//  Authentic Anki SM-2 Algorithm & Cloud-Sync Ready
// ================================================================
...
function previewIntervals(card) {
    ...
    const learningSteps = s.learningSteps || [1, 10]; // [1, 10]
...
function scheduleCard(card, rating) {
    ...
    if (state === 'new' || state === 'learning') {
        ...
        if (rating === 0) { // Again
            ...
            // Immediate Graduate with Easy interval
            state = 'review';
...
async function generateAiFlashcards(promptText, count = 10) {
    ...
    const systemPrompt = `You are an expert tutor and flashcard creator. 
Create exactly ${count} concise, high-yield spaced repetition flashcards from the user's text.
Return ONLY valid JSON matching this schema:
[
  {"front": "Clear question or prompt", "back": "Precise answer or explanation", "tags": ["tag1", "tag2"]}
]`;

    if (!apiKey) {
        // High quality offline fallback generator if no key is entered
        const lines = promptText.split('\n').filter(l => l.trim().length > 5);
        const cards = [];
        for (let i = 0; i < Math.min(count, Math.max(3, lines.length)); i++) {
            const line = lines[i] || `Key Concept ${i + 1} from notes`;
            cards.push({
                front: line.includes(':') ? line.split(':')[0].trim() : `What is the core concept of: "${line.slice(0, 40)}..."?`,
                back: line.includes(':') ? line.split(':')[1].trim() : line,
                tags: ['ai-generated']
            });
        }
        return cards;
    ...
    try {
        ...
        throw new Error('Failed to generate cards with Gemini API: ' + e.message);
    }
    return [];
}
...
function renderDecks() {
    ...
    sortedDecks.forEach(deck => {
        ...
        cardEl.innerHTML = `
            <div class="deck-item-left" data-deck-id="${deck.id}">
        ...
        cardEl.querySelector('[data-study]').addEventListener('click', () => startReview(deck.id));
...
function setupEventListeners() {
    ...
    document.getElementById('saveEditCardBtn')?.addEventListener('click', async () => {
        ...
        if (card) {
            ...
            card.tags = document.getElementById('editCardTags').value.split(',').map(t => t.trim());
            await updateCard(card);
