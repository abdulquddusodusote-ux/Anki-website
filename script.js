...
function hasSubdecks(deckName) {
    const prefix = deckName + '::';
    return App.decks.some(d => d.name.startsWith(prefix));
}

function isDeckHiddenByParent(deckName) {
...
function renderDecks() {
    ...
    sortedDecks.forEach(deck => {
        ...
        if (isDeckHiddenByParent(deck.name)) {
            return;
        }

        const counts = getCardCounts(deck.id);
        const depth = (deck.name.match(/::/g) || []).length;
        const hasChildren = hasSubdecks(deck.name);
